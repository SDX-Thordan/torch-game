...
| metric | value |
| --- | --- |
| treasury | 38000 → 156405 cr (+118405, ~4×) |
| actions | 178 over 4% of ticks |
| pacing | 3464 ticks pending · longest idle 95 ticks |
| campaign | The Gate · gate 100% · 3 ascent(s) |
| gate reached | tick 2341 (~97 days) |
| CEO level | 13 · techs 0 |
| traffic | 166 flew, 146 arrived, 17 cut, 17 shortages |
| act-now alerts | 17 raised, 0 answered |
...
- **[GOOD]** _Economy_ — Turned a profit hands-on/over time: 38000 → 156405 cr (+118405, ~4×).
- **[GOOD]** _Agency_ — Acted on 4% of ticks (178 actions), but the dead time is fast-forwardable: the longest stretch with nothing pending was 95 ticks (~3 days, ~3 s at 24×). With time-compression + auto-pause-on-exception (§28), the player compresses the quiet and is stopped only when an act-now alert fires.
- **[NOTE]** _Alert feed_ — 17 act-now shortages were raised but none were acted on. The ExploitShortage verb needs matching cargo already on hand to exercise — there's no one-press path from the alert to the trade.
...
| Expansionist | **84** | 100 | 98 | 60 | 100 | 50 | 92 |
...
- **Expansionist** █████████· 84/100 — weakest is _Stakes_ (50/100): treasury dip 42%, 5 ships lost, rep low -392, pressure peak 29
...
- **[GOOD]** _UI · wiring_ — Every one of the shell's 163 sim calls resolves to a real binding — no phantom calls that would break at runtime (GDScript wouldn't catch them until that path runs).
- **[NOTE]** _UI · coverage_ — The shell wires 77% of the 211 exposed bindings; 48 are never referenced (e.g. ["admin_load", "alert_threshold", "alpha", "blueprint_discover", "blueprint_known_count", "ceo_branch_name"]). Some are deliberately read-only or future, but a verb the shell never calls is a capability the player can't reach.
