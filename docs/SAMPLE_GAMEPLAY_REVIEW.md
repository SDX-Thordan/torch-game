...
| metric | value |
| --- | --- |
| bindings exposed | 313 |
| wired by the shell | 255 (81%) |
...
- **[GOOD]** _UI · wiring_ — Every one of the shell's 255 sim calls resolves to a real binding — no phantom calls that would break at runtime (GDScript wouldn't catch them until that path runs).
- **[NOTE]** _UI · coverage_ — The shell wires 81% of the 313 exposed bindings; 58 are never referenced (e.g. ["admin_load", "alert_threshold", "ally_count", "alpha", "blueprint_discover", "blueprint_known_count"]). Some are deliberately read-only or future, but a verb the shell never calls is a capability the player can't reach.
