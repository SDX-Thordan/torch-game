...
| metric | value |
| --- | --- |
| bindings exposed | 225 |
| wired by the shell | 175 (77%) |
...
- **[NOTE]** _UI · coverage_ — The shell wires 77% of the 225 exposed bindings; 50 are never referenced (e.g. ["admin_load", "alert_threshold", "ally_count", "alpha", "blueprint_discover", "blueprint_known_count"]). Some are deliberately read-only or future, but a verb the shell never calls is a capability the player can't reach.
