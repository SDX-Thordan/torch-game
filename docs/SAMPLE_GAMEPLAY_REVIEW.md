...
Seed **7**, **4000** ticks (~166 days) per persona. Generated by `torch-qa` driving the deterministic core through 8 play styles. Same seed ⇒ same review.
...
| metric | value |
| --- | --- |
| bindings exposed | 252 |
| wired by the shell | 196 (77%) |
...
- **[GOOD]** _UI · wiring_ — Every one of the shell's 196 sim calls resolves to a real binding — no phantom calls that would break at runtime (GDScript wouldn't catch them until that path runs).
- **[NOTE]** _UI · coverage_ — The shell wires 77% of the 252 exposed bindings; 56 are never referenced (e.g. ["admin_load", "alert_threshold", "ally_count", "alpha", "blueprint_discover", "blueprint_known_count"]). Some are deliberately read-only or future, but a verb the shell never calls is a capability the player can't reach.
