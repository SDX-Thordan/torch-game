...
| metric | value |
| --- | --- |
| bindings exposed | 175 |
| wired by the shell | 130 (74%) |
...
- **[GOOD]** _UI · wiring_ — Every one of the shell's 130 sim calls resolves to a real binding — no phantom calls that would break at runtime (GDScript wouldn't catch them until that path runs).
- **[NOTE]** _UI · coverage_ — The shell wires 74% of the 175 exposed bindings; 45 are never referenced (e.g. ["alert_threshold", "alpha", "blueprint_discover", "blueprint_known_count", "ceo_branch_name", "ceo_gain_xp"]). Some are deliberately read-only or future, but a verb the shell never calls is a capability the player can't reach.
