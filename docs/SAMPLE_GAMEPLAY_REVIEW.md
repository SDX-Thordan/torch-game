...
| metric | value |
| --- | --- |
| bindings exposed | 208 |
| wired by the shell | 160 (76%) |
...
- **[GOOD]** _UI · wiring_ — Every one of the shell's 160 sim calls resolves to a real binding — no phantom calls that would break at runtime (GDScript wouldn't catch them until that path runs).
- **[NOTE]** _UI · coverage_ — The shell wires 76% of the 208 exposed bindings; 48 are never referenced (e.g. ["admin_load", "alert_threshold", "alpha", "blueprint_discover", "blueprint_known_count", "ceo_branch_name"]). Some are deliberately read-only or future, but a verb the shell never calls is a capability the player can't reach.
