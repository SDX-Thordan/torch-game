...
- **[GOOD]** _Fun · breadth_ — Several distinct play styles are engaging (["Logistician", "Privateer", "Tycoon"] all score ≥50/100). Depth-of-decision survives the choice of approach — no single dominant strategy starves the others.
...
| metric | value |
| --- | --- |
| bindings exposed | 176 |
| wired by the shell | 131 (74%) |
...
- **[GOOD]** _UI · wiring_ — Every one of the shell's 131 sim calls resolves to a real binding — no phantom calls that would break at runtime (GDScript wouldn't catch them until that path runs).
- **[NOTE]** _UI · coverage_ — The shell wires 74% of the 176 exposed bindings; 45 are never referenced (e.g. ["alert_threshold", "alpha", "blueprint_discover", "blueprint_known_count", "ceo_branch_name", "ceo_gain_xp"]). Some are deliberately read-only or future, but a verb the shell never calls is a capability the player can't reach.
